...
export interface paths {
  ...
  '/api/v1/courses/{id}': {
     ...
     * @description Admin-only. Updates any combination of the fields on
...
export interface components {
  schemas: {
     ...
     *     The shape is enforced by the server (the `Slug` value object), not by this schema: a `pattern` here would have to be a Unicode property expression, which the contract-test generator cannot build values for, and that made the gate fail at random depending on which operations a stateful scenario happened to chain. An invalid slug is answered 422 by the domain rather than 400 by the request validator.
    ...
    LessonOutlineItem: {
      ...
      /** @description Whether the lesson has a transcript track — a sidecar subtitle, or a generated transcript in the instance's configured transcription language. Same union `LessonDto.subtitles` uses; this is a presence flag rather than the language list because a generated transcript's language is not yet stable across instances. */
     ...
     * @enum {string}
     */
    TranscriptionStatus: 'running' | 'succeeded' | 'failed' | 'cancelled' | 'interrupted';
    /**
    ...
    UpdateCourseRequest: {
      ...
      posterUrl?: string | null;
      /** @description Average rating 0–5. `null` on both rating fields clears the rating; `null` here alone, with `ratingCount` set, is read as 0. */
      ratingAverage?: number | null;
      /** @description Number of ratings. `null` on both rating fields clears the rating; `null` here alone, with `ratingAverage` set, is read as 0. */
      ratingCount?: number | null;
...
export interface operations {
  ...
  listCourses: {
    parameters: {
      query?: {
        ...
        /**
         * @description Filter by total course runtime — the sum of every lesson's duration,
        ...
        /** @description Filter to courses linked to this instructor. An id that matches no instructor yields an empty list rather than an error. */
        instructorId?: string;
        /**
  ...
  createBookmark: {
    ...
    responses: {
      /** @description Replay of an already-applied `idempotencyKey` — returns the bookmark the first call created; nothing new was made. */
      200: {
        headers: {
          [name: string]: unknown;
        };
