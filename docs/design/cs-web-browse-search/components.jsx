...
const SEARCH_LESSON_RESULTS = [
  {
    ...
    course: 'Distributed Systems Foundations',
    section: 'Section 03 · Replication',
    accent: 'teal',
    duration: 26 * 60 + 18,
    snippet:
      'A <mark>quorum</mark> read with R+W>N guarantees you see the latest write — when the network cooperates.',
  },
  {
    id: 'L2',
    ...
    accent: 'teal',
    duration: 18 * 60 + 24,
    snippet: 'Sloppy <mark>quorums</mark> trade durability for availability during partitions.',
  },
  {
    id: 'L3',
    title: 'Read repair and anti-entropy',
    course: 'Distributed Systems Foundations',
    section: 'Section 04 · Consistency',
    accent: 'teal',
    duration: 22 * 60 + 45,
    snippet:
      'When <mark>quorum</mark> reads detect a stale replica, the coordinator writes back the fresh value.',
  },
  {
    id: 'L4',
    title: 'Indexes and quorum reads in Postgres',
    course: 'Modern PostgreSQL — Performance Deep Dive',
    section: 'Section 06 · Replication',
    accent: 'indigo',
    duration: 19 * 60 + 10,
    snippet:
      'Logical replication slots can power <mark>quorum</mark>-style read distribution across replicas.',
  },
];
...
const FilterPanel = ({ values, onChange, instructorSearch = false }) => {
  ...
  const filteredInstructors = INSTRUCTORS.filter(
    (i) => !iSearch || i.name.toLowerCase().includes(iSearch.toLowerCase()),
  );
    ...
    <div className="fp" role="group" aria-label="Filters">
      <div className="fp-section" data-open={open.status}>
        ...
        <div className="fp-options">
          {[
            ['not-started', 'Not started', '152'],
            ['in-progress', 'In progress', '38'],
            ['completed', 'Completed', '61'],
          ].map(([k, l, c]) => (
            <label key={k} className="fp-opt">
              <input
                type="checkbox"
                checked={(values.status || []).includes(k)}
                onChange={() => toggleArr('status', k)}
              />
        ...
        <div className="fp-options">
          {LIBRARIES.map((lib) => (
            <label key={lib.id} className="fp-opt">
              <input
                type="checkbox"
                checked={(values.library || []).includes(lib.id)}
                onChange={() => toggleArr('library', lib.id)}
              />
      ...
      <div className="fp-section" data-open={open.duration}>
        ...
        <div className="fp-options">
          {[
            ['lt5', 'Under 5 hours', '12'],
            ['5to10', '5–10 hours', '21'],
            ['10to20', '10–20 hours', '14'],
            ['gt20', 'Over 20 hours', '4'],
          ].map(([k, l, c]) => (
            <label key={k} className="fp-opt">
              <input
                type="checkbox"
                checked={(values.duration || []).includes(k)}
                onChange={() => toggleArr('duration', k)}
              />
              <span className="fp-opt-label">{l}</span>
              <span className="fp-opt-count">{c}</span>
            </label>
        ...
        <div className="fp-options">
          {filteredInstructors.slice(0, 8).map((i) => (
            <label key={i.id} className="fp-opt">
              <input
                type="checkbox"
                checked={(values.instructor || []).includes(i.id)}
                onChange={() => toggleArr('instructor', i.id)}
              />
          ...
          {filteredInstructors.length > 8 && (
            ...
            >
              Show all {filteredInstructors.length}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="sr-title">
        <Highlight>
          {course.title.replace(
            /(quorum|postgres|color|distributed|systems)/i,
            (m) => `<mark>${m}</mark>`,
          )}
        </Highlight>
      </div>
...
const LessonSearchItem = ({ lesson }) => (
  <div className="sr-item" tabIndex={0} role="link">
    ...
    <div className="sr-body">
      <div className="sr-title">
        <Highlight>{lesson.title}</Highlight>
      </div>
      <div className="sr-context">
      ...
      <div className="sr-snippet">
        <Highlight>{lesson.snippet}</Highlight>
      </div>
    </div>
