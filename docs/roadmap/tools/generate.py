...
- docs/roadmap/tasks/<ID>.md       — one file per story (one per registry entry)
...
add(
    ...
    substeps=[
        ...
        "create empty `apps/{backend,web,mobile}` and `packages/{specs,api-client-ts,api-client-dart,design-tokens,eslint-config,tsconfig}`",
...
add(
    ...
    title="OpenAPI + AsyncAPI workspace, Spectral lint, bundle scripts",
    duration=1, stage="A",
    deps=["E01-F01-S02"],
    spec_refs=["AGENTS.md §6.3", "DESIGN.md §3"],
    goal="Author-and-bundle pipeline for `packages/specs` — two spines: OpenAPI 3.1 for REST and AsyncAPI 3.0 for Centrifugo channels.",
    acceptance=[
    ...
    substeps=[
        "create `packages/specs/openapi/openapi.yaml` with `info`, `servers`, empty `paths`/`components`",
        "create `packages/specs/asyncapi/centrifugo.yaml` as the AsyncAPI 3.0 skeleton",
        ...
        "add AsyncAPI bundle step emitting `dist/asyncapi.json`",
        "add npm scripts `spec:validate`, `spec:bundle`",
    ],
)
...
add(
    ...
    title="Generate `@app/api-client-ts` from the spec",
    duration=2, stage="A",
    deps=["E02-F01-S02"],
...
add(
    ...
    epic="E02", epic_title="API contracts as the spine",
    feature="F02", feature_title="Codegen pipeline",
    title="Generate NestJS DTOs as `@app/api-client-ts/server` subpath",
    duration=2, stage="A",
    deps=["E02-F01-S02"],
    spec_refs=["AGENTS.md §7.3", "DESIGN.md §3.2"],
    goal="NestJS DTO classes with `class-validator` decorators inferred from schema constraints, exported via the `@app/api-client-ts/server` subpath export.",
    acceptance=[
...
add(
    ...
    epic="E02", epic_title="API contracts as the spine",
    feature="F02", feature_title="Codegen pipeline",
    title="Generate Dart Dio client as `@app/api-client-dart`",
    duration=2, stage="A",
    deps=["E02-F01-S02"],
    spec_refs=["DESIGN.md §3.2", "DESIGN.md §8"],
    goal="A typed Dart client compatible with Dio that the mobile app consumes for all `/api/v1/*` calls, living in `packages/api-client-dart`.",
    acceptance=[
...
add(
    ...
    epic="E02", epic_title="API contracts as the spine",
    feature="F02", feature_title="Codegen pipeline",
    title="`pnpm spec:codegen` aggregate + contract-test scaffold",
    duration=1, stage="A",
    deps=["E02-F02-S01", "E02-F02-S02", "E02-F02-S03"],
    spec_refs=["AGENTS.md §8"],
    goal="One command regenerates all three clients; a Dredd/Prism contract-test scaffold validates the bundled spec against the running backend.",
    acceptance=[
...
add(
    ...
    title="Import bundle tokens.json verbatim; wire custom design-tokens pipeline",
    duration=2, stage="A",
    deps=["E00-F01-S01", "E01-F01-S02"],
    spec_refs=["docs/design/shared/tokens.json", "AGENTS.md §7.9"],
    goal="One token source, three outputs (CSS + TS + Dart), all faithful to the bundle.",
    acceptance=[
...
add(
    ...
    epic="E03", epic_title="Design tokens",
    feature="F01", feature_title="Token source",
    title="Tailwind/Nuxt UI and Flutter theme adapters + Foundations canvas",
    duration=2, stage="A",
    deps=["E03-F01-S01"],
...
add(
    ...
    title="Bootstrap apps/backend with Prisma, modules skeleton, and shared kernel",
    duration=2, stage="A",
    deps=["E02-F02-S04"],
    spec_refs=["AGENTS.md §7.2", "DESIGN.md §5"],
    goal="An app skeleton that compiles, with bounded-context module scaffolding and the shared kernel (Branded IDs + DomainError) ready.",
    acceptance=[
...
add(
    ...
    goal="Better Auth boots, manages its own Prisma tables, exposes `/api/v1/auth/*`, issues bearer tokens via `Set-Auth-Token`.",
    acceptance=[
...
add(
    ...
    goal="Runtime validation of request/response against the bundled spec; `/api/v1/auth/*` and `/healthz` exempted.",
    acceptance=[
    ...
    substeps=[
        "mount middleware with `ignorePaths: /^\\/(api\\/v1\\/auth|healthz|readyz)/`",
        "map validator errors to `Problem` shape (RFC 9457)",
...
add(
    ...
    substeps=[
        "edit `packages/specs/openapi/openapi.yaml` (paths for libraries + component schema)",
...
add(
    ...
    feature="F01", feature_title="Nuxt + Nuxt UI shell",
    title="Bootstrap Nuxt SPA with Nuxt UI v4, Tailwind 4, and tokens",
    duration=2, stage="A",
    deps=["E03-F01-S02", "E04-F02-S03"],
    spec_refs=["AGENTS.md §7.5", "DESIGN.md §2.9b", "DESIGN.md §7"],
    goal="A Nuxt 4 SPA (`ssr: false`) with Nuxt UI v4 and Tailwind 4 themed by the generated token CSS variables.",
    acceptance=[
...
add(
    ...
    epic="E11", epic_title="Web foundations",
    feature="F01", feature_title="Nuxt + Nuxt UI shell",
    title="Wire @app/api-client-ts with bearer interceptor + auth store",
    duration=2, stage="A",
    deps=["E11-F01-S01"],
    spec_refs=["AGENTS.md §7.4", "DESIGN.md §7.3", "DESIGN.md §10.5"],
    goal="Every HTTP call goes through `@app/api-client-ts`; bearer token resolved from a Pinia auth store backed by Better Auth client.",
    acceptance=[
...
add(
    ...
    epic="E11", epic_title="Web foundations",
    feature="F01", feature_title="Nuxt + Nuxt UI shell",
    title="Auth store + minimal sign-in/setup pages",
    duration=2, stage="A",
    deps=["E11-F01-S02"],
    spec_refs=["PRD.md FR-AUTH-01"],
    goal="First-run wizard creates the admin; `/sign-in` works against the real backend via the Pinia auth store.",
    acceptance=[
    ...
    notes="The *real* sign-in + setup design lands when `cs-web-auth` is prototyped (Stage B in E14-F02).",
    substeps=[
...
add(
    ...
    title="Configure Storybook for Vue 3 + Nuxt UI v4 + tokens",
    duration=1, stage="A",
    deps=["E11-F01-S01"],
...
add(
    ...
    acceptance=["All jobs run on PR; required for merge", "Codegen-no-diff job: runs `pnpm spec:codegen` then `git diff --exit-code`"],
)
...
add(
    id="E24-F01-S01",
    epic="E24", epic_title="Realtime / Centrifugo",
    feature="F01", feature_title="Realtime token + channels",
    title="AsyncAPI Centrifugo channels + POST /api/v1/realtime/token",
    duration=2, stage="A",
    deps=["E04-F02-S03"],
    spec_refs=["AGENTS.md (Centrifugo conventions)", ".claude/CLAUDE.md (URLs section)"],
    goal="Realtime spine: clients receive a short-lived Centrifugo JWT (TTL ≤5 min) from `POST /api/v1/realtime/token`; AsyncAPI 3.0 documents the channels they subscribe to.",
    acceptance=[
    ...
    spec_diff="openapi.yaml + asyncapi/centrifugo.yaml",
    codegen="yes",
    tests="integration spec for the controller; AsyncAPI lint via Spectral",
    substeps=[
...
# ----- Card status (the cards, not the registry, are the source of truth) ----

# The registry in this file is a snapshot of what the cards said the day they
# were seeded; `refuse_if_seeded()` exists because of that. Progress lives in
# `tasks/<ID>.md` under `**Status:**`, so the Gantt reads it from there rather
# than carrying a second, immediately-stale copy.
STATUS_DONE = "\u2705"
STATUS_CANCELLED = "\u274c"


def card_status(story_id):
    """Return 'done' | 'cancelled' | 'open' for one story, read from its card."""
    card = TASKS / f"{story_id}.md"
    if not card.exists():
        return "open"
...
def write_roadmap():
    ...
    status = {s["id"]: card_status(s["id"]) for s in S}
    cancelled = {sid for sid, st in status.items() if st == "cancelled"}
    ...
    for epic in sorted(by_epic.keys()):
        ...
        for s in rows:
            ...
            # `crit` encodes Stage B (a design pre-step is required); it is
            # orthogonal to progress, so both tags can appear on one row.
            tags = ["crit"] if s["stage"] == "B" else []
            tags.append("done" if status[s["id"]] == "done" else "active")
            tag = ", ".join(tags)
            # A cancelled story is not rendered, so nothing may point at it.
            deps = [d for d in s["deps"] if d not in cancelled]
            if deps:
                dep_ids = " ".join(gantt_id(d) for d in deps)
                out.append(
                    f"    {label:60s} :{tag}, {sid}, after {dep_ids}, {s['duration']}d\n"
                )
            else:
                out.append(
...
def write_readme():
    ...
    parts = [
        "# CourseShelf — Roadmap",
        "",
        f"This directory holds the executable plan for CourseShelf v1: {total} stories",
...
if __name__ == "__main__":
    # `--roadmap-only` skips the seeder entirely: ROADMAP.md is derived output
    # with nothing hand-maintained in it, so the guard that protects the cards
    # does not apply. `--check` turns it into a CI gate — it writes nothing and
    # fails when the committed chart disagrees with the cards.
    if "--roadmap-only" in sys.argv:
        ...
        if "--check" in sys.argv:
            ...
            if before != after:
                if before is None:
                    raise SystemExit(f"{target} is missing; run --roadmap-only")
                target.write_text(before)  # leave the tree as we found it
                raise SystemExit(
                    "ROADMAP.md is out of date with docs/roadmap/tasks/*.md.\n"
                    "Run: python3 docs/roadmap/tools/generate.py --roadmap-only"
                )
            print("ROADMAP.md matches the cards.")
        else:
            write_roadmap()
