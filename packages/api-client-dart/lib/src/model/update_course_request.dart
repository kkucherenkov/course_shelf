...
/// Payload for updating course metadata. All fields are optional, but at least one of `title`, `description`, `slug`, `instructorIds`, `studioIds`, `tagIds`, `posterUrl`, `level`, `language`, `releaseDate`, `sourceUpdatedAt`, `ratingAverage`, `ratingCount`, or `externalIds` must be present.  `ratingAverage` and `ratingCount` are supplied together or not at all — passing one key without the other is a 400. Within a supplied pair, `null` on both sides clears the rating; `null` on just one side reads as 0 (a rating with a known-empty sample, or a sample count with no computed average, is still a complete fact — it's the missing pairing that would be ambiguous, not the zero).  **Set-replace semantics for relation arrays:** `null` means \"leave the existing set alone\"; `[]` (empty array) means \"remove all links\"; a non-empty array replaces the full set with exactly the listed ids. 
///
/// Properties:
/// * [title] 
/// * [description] 
/// * [slug] - URL-safe slug. 1–100 characters, lowercase Unicode letters, digits, and hyphens; cannot start or end with a hyphen. Unique within a library. Non-ASCII scripts are kept as themselves rather than transliterated, so `Графы и комбинаторика` slugs to `графы-и-комбинаторика` — a course is only ever addressed by id, and the slug's jobs are uniqueness within the library and human recognition. Values are NFC-normalised, so two encodings of the same visual title are the same slug. The shape is enforced by the server (the `Slug` value object), not by this schema: a `pattern` here would have to be a Unicode property expression, which the contract-test generator cannot build values for, and that made the gate fail at random depending on which operations a stateful scenario happened to chain. An invalid slug is answered 422 by the domain rather than 400 by the request validator.
/// * [instructorIds] 
/// * [studioIds] 
/// * [tagIds] 
/// * [level] 
