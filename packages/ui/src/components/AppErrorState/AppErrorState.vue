<script setup lang="ts">
  ...
  withDefaults(
    ...
    { icon: 'alert', body: undefined, headingLevel: 2 },
  );
</script>
...
<template>
  <div class="app-error-state" role="alert">
    ...
    <component :is="`h${headingLevel}`" class="app-error-state__title">
      {{ title }}
    </component>
    ...
    <div v-if="$slots['action'] || $slots['actions']" class="app-error-state__action">
      <slot name="action">
        <slot name="actions" />
      </slot>
    </div>
