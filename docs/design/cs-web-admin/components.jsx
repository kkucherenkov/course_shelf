...
const StatCard = ({ icon, label, num, meta, delta, deltaDir, isError, loading }) => {
  ...
  return (
    <div className={`stat-card ${isError ? 'is-error' : ''}`}>
      ...
      <div className="meta">
        {delta && (
          <span className={deltaDir === 'down' ? 'delta-down' : 'delta-up'}>
            <Icon name={deltaDir === 'down' ? 'chevron-down' : 'chevron-up'} size={11} />
            {delta}
          </span>
        )}
        {meta && <span>{meta}</span>}
  ...
  return (
    <div className="lib-row" data-bp={bp} tabIndex={0} role="button" onClick={onClick}>
      ...
      <div style={{ minWidth: 0 }}>
        ...
        {bp === 'xs' && (
          <div className="lib-name-sub">
            <span>{library.courses} courses</span>
            <span>·</span>
      ...
      {bp !== 'xs' && (
        <div
          ...
          role="button"
          tabIndex={0}
          aria-label={`Copy path ${library.path}`}
        >
      ...
      {bp === 'lg' && (
        <div className="lib-meta-meta">
          <span className="num">{library.courses}</span> courses
        </div>
      )}
...
const RoleChip = ({ role, onChange, readOnly }) => {
  ...
  return (
    <span
      ...
      aria-label={`Role: ${labels[role]}${onChange ? ', click to change' : ''}`}
    >
...
const UserRow = ({ user, bp, onChangeRole, onClick }) => {
    ...
    return (
      <div className="user-card-xs" tabIndex={0} role="button" onClick={onClick}>
        <div
          ...
          style={{ background: user.avatarBg || 'var(--surface-3)' }}
        >
          {user.initials}
        </div>
        <div style={{ minWidth: 0 }}>
  ...
  return (
    <div className="user-row" data-bp={bp} tabIndex={0} role="button" onClick={onClick}>
      <div className="avatar avatar-md" style={{ background: user.avatarBg || 'var(--surface-3)' }}>
        {user.initials}
      </div>
      <div style={{ minWidth: 0 }}>
        <div className="user-name">{user.name}</div>
      ...
      {bp === 'lg' && (
        <div className="user-email">
          {user.libraries} libraries · {user.minutes} min watched
        </div>
      )}
    ...
    <>
      <div className={`perm-row ${hasOv ? 'has-overrides' : ''}`} data-bp={bp}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div className="perm-row-name">
              {library.name}
              {hasOv && (
                <span className="chip chip-info" style={{ fontSize: 11, padding: '1px 6px' }}>
                  {overrides.length} override{overrides.length > 1 ? 's' : ''}
                </span>
              )}
            </div>
            <div className="perm-row-meta">
              {library.courses} courses · {library.size}
            </div>
          </div>
        </div>
        <div className="level-toggle" role="group" aria-label={`Access level for ${library.name}`}>
          <button
            data-level="read"
            aria-selected={level === 'read'}
            onClick={() => onChangeLevel(library.id, 'read')}
          >
            Read
          </button>
          <button
            data-level="none"
            aria-selected={level === 'none'}
            onClick={() => onChangeLevel(library.id, 'none')}
          >
            None
          </button>
        </div>
        {bp !== 'xs' && (
          <button
            className="btn btn-ghost btn-icon btn-sm"
            onClick={() => hasOv && setOpen(!open)}
            aria-label={hasOv ? 'Toggle overrides' : 'No overrides'}
            disabled={!hasOv}
          >
            <Icon
              name="chevron-down"
          ...
          {overrides.map((o) => (
            <div key={o.id} className="perm-override-row" data-bp={bp}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center', minWidth: 0 }}>
                <div
                  style={{
                    ...
                    height: 18,
                    borderRadius: 3,
                    background: COVER[o.accent] || 'var(--surface-3)',
                    flexShrink: 0,
                  }}
                />
              ...
              <div className="level-toggle">
                <button data-level="read" aria-selected={o.level === 'read'}>
                  Read
                </button>
                <button data-level="none" aria-selected={o.level === 'none'}>
                  None
                </button>
              </div>
              <button className="btn btn-ghost btn-icon btn-sm" aria-label="Remove override">
                <Icon name="x" size={14} />
              </button>
            </div>
          ))}
        ...
        <div className="adm-sheet-body">
          <div>
            ...
            <div className="level-toggle" style={{ width: '100%' }}>
              <button
                data-level={scope === 'library' ? 'read' : 'none'}
              ...
              <button
                data-level={scope === 'course' ? 'read' : 'none'}
                aria-selected={scope === 'course'}
          ...
          {scope === 'library' && (
            <div>
              ...
              <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 2 }}>
                {[
                  { id: 'cs', n: 'Computer Science', c: 14 },
                  { id: 'art', n: 'Art', c: 7 },
                  { id: 'db', n: 'Databases', c: 5 },
                ].map((l) => (
                  <label
                    key={l.id}
                    className="fp-opt"
                  ...
                  >
                    ...
                    <span className="t-mono t-mute" style={{ fontSize: 11 }}>
                      {l.c}
                    </span>
                  </label>
                ))}
          ...
          {scope === 'course' && (
            <div>
              ...
              <p className="t-caption t-mute" style={{ marginTop: 8 }}>
                Course overrides take precedence over library-level grants for this user.
              </p>
            </div>
          )}
          <div>
          ...
          <div
            ...
            style={{ padding: 12, fontSize: 12, lineHeight: '18px' }}
          >
            <Icon name="info" size={14} />
            <div className="banner-body">
              Marco Bucci will gain Read access to <b>Computer Science</b> immediately. They'll see
              14 new courses on their shelf.
            </div>
          </div>
        </div>
