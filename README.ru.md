...
Это значения по умолчанию. Каждый порт и имя проекта задаются переменными
окружения (`CS_BACKEND_PORT`, `CS_WEB_PORT`, `COMPOSE_PROJECT_NAME`, ...),
поэтому второй checkout может поднять свой стек рядом с первым — см.
...
и, позже, превью из сканирования, плюс собственноручно написанные определения
скраперов в `derived/scrapers/`) и `WHISPER_MODEL_DIR` (по умолчанию
...
Первый пользователь, который вызывает `POST /api/v1/setup/owner`, становится владельцем; последующие попытки возвращают 409. SPA автоматически перенаправляет свежую установку на мастер `/sign-up` — откройте `http://localhost:8080`, введите email и пароль, шаг 1 сделает эту учётную запись администратором и передаст управление дашборду.
...
specs/
  tasks/            active/ (по файлу на задачу) + done/ (архив)
