...
const FrameSide = ({ active, bp }) => {
  ...
  const main = [['home','Home','home'],['browse','Browse','library'],['search','Search','search']];
  const admin = [['dashboard','Dashboard','grid'],['libraries','Libraries','folder'],['users','Users','users'],['permissions','Permissions','key']];
  return (
...
const PAGE_META = {
  dashboard:    { title: 'Dashboard',   sub: 'Overview of every library, scan, and user.', crumbs: ['Admin', 'Dashboard'],   url: '/admin' },
  libraries:    { title: 'Libraries',   sub: '6 libraries · 1.32 TB total.',                crumbs: ['Admin', 'Libraries'],   url: '/admin/libraries' },
  'library-detail': { title: 'Computer Science', sub: '14 courses · scanned 12 minutes ago.', crumbs: ['Admin', 'Libraries', 'Computer Science'], url: '/admin/libraries/cs' },
  users:        { title: 'Users',       sub: '42 users · 38 active.',                       crumbs: ['Admin', 'Users'],       url: '/admin/users' },
  permissions:  { title: 'Permissions · Marco Bucci', sub: 'Library and per-course access for this user.', crumbs: ['Admin', 'Users', 'Marco Bucci', 'Permissions'], url: '/admin/users/marco/permissions' },
};
...
const AdmFrame = ({ bp, theme, screen, state }) => {
  ...
  const primaryAction = (() => {
    if (screen === 'libraries') return <button className="btn btn-primary btn-sm" onClick={() => setSheet(true)}><Icon name="plus" size={12}/>Add library</button>;
    if (screen === 'users')     return <button className="btn btn-primary btn-sm"><Icon name="plus" size={12}/>Add user</button>;
    return null;
  })();
  ...
  return (
    <div className="adm-frame" data-mode={theme} style={{width: w}}>
      ...
      <div className="adm-frame-body">
        <div className="adm-shell" data-bp={bp}>
          ...
          <div className="adm-main">
            <header className="adm-topbar">
              {bp === 'xs' && <button className="btn btn-ghost btn-icon btn-sm" aria-label="Menu"><Icon name="menu" size={16}/></button>}
            ...
            <div className="adm-content">
              {screen !== 'library-detail' && screen !== 'permissions' && (
                <div className="adm-page-h">
                  ...
                  {bp !== 'xs' && primaryAction && <div className="adm-page-actions">{primaryAction}</div>}
                </div>
              )}

              {screen === 'dashboard'   && <DashboardContent bp={bp} state={state}/>}
              {screen === 'libraries'   && <LibrariesContent bp={bp} state={state} onAdd={() => setSheet(true)}/>}
              {screen === 'library-detail' && <LibraryDetail bp={bp} state={state}/>}
              {screen === 'users'       && <UsersContent bp={bp} state={state}/>}
              {screen === 'permissions' && <PermissionsContent bp={bp} state={state} onAddGrant={() => setSheet(true)}/>}
            </div>
            {bp === 'xs' && primaryAction && (
        ...
        {sheet && screen === 'permissions' && <AddGrantSheet bp={bp} onClose={() => setSheet(false)}/>}
        {sheet && screen === 'libraries' && <AddLibrarySheet bp={bp} onClose={() => setSheet(false)}/>}
      </div>
    </div>
  );
};
  ...
  <div className="adm-sheet-backdrop" data-bp={bp} onClick={onClose}>
    <div className="adm-sheet" onClick={e => e.stopPropagation()}>
      ...
      <div className="adm-sheet-body">
        <div>
          <div className="field-label" style={{marginBottom:6}}>Name</div>
          <input className="input" placeholder="e.g. Computer Science" defaultValue="Computer Science"/>
        </div>
        <div>
          <div className="field-label" style={{marginBottom:6}}>Root path on disk</div>
          <div className="input-with-icon">
            <Icon name="folder" size={14}/>
            <input className="input" placeholder="/srv/courses/…" defaultValue="/srv/courses/computer-science"/>
          </div>
          <div className="field-help" style={{marginTop:6, display:'flex', gap:6, alignItems:'center'}}>
            <Icon name="check" size={12} style={{color:'var(--success)'}}/> Path readable · 14 candidate course folders found
          </div>
        </div>
        <div>
          <div className="field-label" style={{marginBottom:6}}>Auto-scan</div>
          <div className="level-toggle" style={{width:'100%'}}>
            <button data-level="read" aria-selected="true" style={{flex:1}}>Daily 03:00</button>
            <button data-level="none" style={{flex:1}}>Manual only</button>
          </div>
        </div>
    ...
    <div className="adm-stage" data-screen-label="01 cs-web-admin">
      <div className="adm-stage-h">
        <div>
          ...
          <h1 className="t-display" style={{margin:'4px 0 0'}}>Admin · web</h1>
        </div>
        <p className="t-mute" style={{margin:0, maxWidth:'70ch', flex:'1 1 320px', minWidth:280}}>
          ...
          (<span className="t-mono">xs · 360</span>, <span className="t-mono">md · 1024</span>, <span className="t-mono">lg · 1440</span>),
          dark + light side by side. Tables collapse to row lists at xs; primary actions stay visible.
        </p>
      </div>
      ...
      {/* LIBRARIES =============================================== */}
      <section>
        <div className="adm-cluster-h">Libraries · default · 6 libraries · mixed scan states (running, warning, failed)</div>
      ...
      {/* USERS =================================================== */}
      <section>
        <div className="adm-cluster-h">Users · default · 7 users with inline role chips · disabled state visible</div>
      ...
      {/* PERMISSIONS ============================================= */}
      <section>
        <div className="adm-cluster-h">Permissions · default · 6 libraries × Read/None toggle · 2 with course overrides</div>
      ...
      <section>
        <div className="adm-cluster-h">Permissions · xs · stacked rows, no inline overrides accordion</div>
