...
`docs/roadmap/tools/generate.py` is a **one-shot seeder, not a generator**. It
seeded the cards once; since then the cards in `tasks/` are the source of
truth and the registry inside the script is a snapshot of what they said on
the day they were written.

A plain run resets every card to `Status: ⬜ Not started`, wipes every
`Completed` / `Result` / deviation note added since, and rewrites TODO.md's
progress line to `0 / N`. `refuse_if_seeded()` blocks it for exactly that
reason, and nothing it destroys is recoverable from the registry — only from
git.

**To mark a story done:** edit its `tasks/<ID>.md` (`**Status:** ✅ Done`, tick
the sub-steps, add `Completed` and `Result`), tick its row in `TODO.md`, and
bump the progress counter by hand.

**The one safe invocation** regenerates only the Gantt chart in `ROADMAP.md`,
which is derived output with nothing hand-maintained in it:

```sh
python3 docs/roadmap/tools/generate.py --roadmap-only
```

CI runs `--roadmap-only --check`, which writes nothing and fails when the
committed chart disagrees with the cards.
