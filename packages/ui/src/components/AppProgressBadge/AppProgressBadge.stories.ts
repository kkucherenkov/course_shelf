...
export const Variants: Story = {
  render: () => ({
    ...
    template: `
      <div style="display: grid; grid-template-columns: 100px repeat(4, 1fr); gap: 16px; align-items: center; padding: 16px;">
        <div></div>
        <div v-for="s in states" :key="s" style="font-size: 12px; color: var(--text-secondary); text-transform: uppercase;">{{ s }}</div>
        <template v-for="v in variants" :key="v">
          <div style="font-size: 12px; color: var(--text-secondary); text-transform: uppercase;">{{ v }}</div>
