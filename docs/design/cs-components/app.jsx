  ...
  return (
    <div className="dc-root" data-screen-label="cs-components">
      <header className="dc-header">
        <div>
          ...
          <p className="t-mute" style={{ margin: '8px 0 0', maxWidth: '68ch' }}>
            Reusable across web and mobile. Every variant in every state — composed from{' '}
            <a href="../cs-foundations/index.html" style={{ color: 'var(--primary)' }}>
      ...
      <Sec
        n={1}
        title="CourseCard"
        sub="Three variants — poster (grids), wide (continue-watching rows), compact (dense lists). Eight states each."
      >
        <div className="dc-grid dc-grid-2">
          <Stage label="Poster · variants & states">
            ...
            <div className="t-caption t-mute" style={{ marginTop: 12 }}>
              States, l→r: in-progress · completed · not-started · locked · in-progress · completed
              · not-started · loading
            </div>
          </Stage>
      ...
      <Sec
        n={2}
        title="LessonRow + SectionHeader"
        sub="Single horizontal row inside the course detail page. Shows lesson position, title, duration, status, materials icon, mobile download state."
      >
        <div className="dc-grid dc-grid-2">
            ...
            <div className="card" style={{ padding: 4 }}>
              ...
              {openSec && (
                <>
                  ...
                  <LessonRow
                    num={6}
                    title="Causal consistency (admin-only)"
                    duration={28 * 60}
                    state="locked"
                  />
                </>
      ...
      <Sec
        n={3}
        title="PlayerChrome"
        sub="Hover-revealed overlay (toggle via the corner button to see minimal mode); mobile landscape with edge gestures; states: playing, buffering, error, end-of-lesson, locked."
      >
        <div className="dc-grid dc-grid-2">
          <Stage label="Web — desktop · playing / overlay & minimal">
            <PlayerChrome context="desktop" state="playing" />
            <div className="t-caption t-mute" style={{ marginTop: 8 }}>
              Click "show minimal" in the corner to see the 3-second-idle state.
            </div>
          </Stage>
      ...
      <Sec
        n={4}
        title="Bookmark & Note"
        sub="Bookmark: timestamp + label, edit/delete on hover, inline add form (no modal). Note: per-lesson markdown editor with auto-save indicator."
      >
        <div className="dc-grid dc-grid-2">
      ...
      <Sec
        n={5}
        title="ProgressBadge"
        sub="Three variants (ring, pill, bar) × four states (not-started, in-progress, completed, locked)."
      >
          ...
          <table className="fs-state-table" style={{ width: '100%' }}>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  <ProgressBadge variant="ring" state="not-started" accent="teal" />
                </td>
                <td>
                  <ProgressBadge
                    variant="ring"
                ...
                <td>
                  <ProgressBadge variant="ring" state="completed" accent="amber" />
                </td>
                <td>
                  <ProgressBadge variant="ring" state="locked" />
                </td>
              </tr>
              <tr>
                <td className="t-body-strong">Pill</td>
                <td>
                  <ProgressBadge variant="pill" state="not-started" />
                </td>
                <td>
                  <ProgressBadge variant="pill" state="in-progress" completed={4} total={12} />
                </td>
                <td>
                  <ProgressBadge variant="pill" state="completed" />
                </td>
                <td>
                  <ProgressBadge variant="pill" state="locked" />
                </td>
              </tr>
              <tr>
                <td className="t-body-strong">Bar</td>
                <td style={{ width: 160 }}>
                  <ProgressBadge variant="bar" state="not-started" />
                </td>
                <td style={{ width: 160 }}>
                  <ProgressBadge variant="bar" state="in-progress" completed={4} total={12} />
                </td>
                <td style={{ width: 160 }}>
                  <ProgressBadge variant="bar" state="completed" />
                </td>
                <td style={{ width: 160 }}>
                  <ProgressBadge variant="bar" state="locked" />
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <Sec
        n={6}
        title="ScanProgressIndicator"
        sub="Used on the admin Library detail page while a scan is running. Live counters; current path truncated; cancel & view-errors affordances."
      >
        <div className="dc-grid dc-grid-2">
          <Stage label="Running">
            <ScanProgress status="running" />
          </Stage>
          <Stage label="Succeeded">
      ...
      <Sec
        n={7}
        title="DownloadRow"
        sub="Mobile downloads manager. Per-lesson row: title, course, size, state, progress, action."
      >
        <Stage label="Every state">
          <div className="card" style={{ padding: 0, maxWidth: 480 }}>
            ...
            />
            <DownloadRow
              lesson="Eigen­vectors deep dive"
              course="Linear Algebra for ML"
              size="142 MB"
              state="failed"
              accent="teal"
      ...
      <footer
        ...
        style={{ paddingTop: 32, borderTop: '1px solid var(--border)', marginTop: 32 }}
      >
