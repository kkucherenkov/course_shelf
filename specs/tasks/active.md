...
- Status: done
- Blockers: —
- Coordination flagged to maintainer (not touched, per lane boundaries):
  - `layouts/default.vue` (a11y-document): admin sidebar now shows two
    ...
    for non-admins now that the route redirects them
  - `settings.vue:12` (audit-regressions): "density (3-up picker)" comment
    is stale now that only two options remain
