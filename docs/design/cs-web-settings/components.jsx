...
const SyncIndicator = ({ state }) => {
  ...
  return (
    ...
    >
      ...
      {state === 'saved' && (
        <>
          <Icon name="check" size={11} className="check" />
          Saved
        </>
      )}
      {state === 'error' && (
        <>
          <Icon name="alert" size={11} className="check" />
          Couldn't save
        </>
      )}
    </span>
...
const SettingRow = ({ label, help, control, syncState = 'idle', danger, bp, vertical }) => (
  <div
    className={`set-row ${vertical ? 'set-row-vertical' : ''}`}
    data-row-sync={syncState}
    data-bp={bp}
...
const DensityPicker = ({ value, onChange }) => (
  <div className="density-picker" role="group" aria-label="Density">
    {[
      ...
      <button key={v} aria-selected={value === v} onClick={() => onChange(v)}>
        {l}
      </button>
    ))}
  </div>
    ...
    return (
      <div className="set-col">
        <div className="set-page-h">
          <div>
            <div className="skel" style={{ height: 24, width: 120 }} />
            <div className="skel" style={{ height: 14, width: 240, marginTop: 8 }} />
          </div>
        </div>
        {[0, 1, 2, 3].map((s) => (
    ...
    <div className="set-col">
      ...
      {state === 'error' && (
        <div className="set-error-banner" role="alert">
          ...
          <div className="body">
            <strong>Couldn't reach the server.</strong> Your changes are kept locally — we'll retry
      ...
      <SettingSection icon="user" title="Profile">
        <SettingRow
          ...
          control={
            <div className="set-avatar-block">
              ...
              <div
                className="set-avatar-actions"
                style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 4 }}
              >
                <button className="btn btn-secondary btn-sm">
                  {state === 'empty' ? 'Upload image' : 'Change image'}
                </button>
                {state !== 'empty' && (
        ...
        <SettingRow
          ...
          control={
            <input className="input set-input" defaultValue={name} placeholder="Your name" />
          }
        />
        <SettingRow
          bp={bp}
          ...
          control={
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              <span className="t-mono" style={{ fontSize: 13, color: 'var(--text)' }}>
                elena@courseshelf.local
              </span>
