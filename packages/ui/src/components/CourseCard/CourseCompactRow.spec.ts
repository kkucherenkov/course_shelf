...
describe('CourseCompactRow', () => {
  ...
  it('renders the title', () => {
    ...
    expect(wrapper.find('.app-course-compact-row__title').text()).toBe('Advanced Vue Patterns');
  });
  ...
  it('renders pct label', () => {
    ...
    expect(wrapper.find('.app-course-compact-row__pct').text()).toBe('33%');
  });
  ...
  it('uses COVER[accent] as thumb background when course.cover is undefined', () => {
    const wrapper = mount(CourseCompactRow, { props: { course: base } });
    const thumb = wrapper.find('.app-course-compact-row__thumb');
  ...
  it('state=auto + completed===lessons → bar fill=100%', () => {
    ...
    expect(wrapper.find('.app-course-compact-row__bar-fill').attributes('style')).toContain(
      'width: 100%',
    );
    expect(wrapper.find('.app-course-compact-row__pct').text()).toBe('100%');
  });
  ...
  it('state=auto + partial → bar fill proportional', () => {
    ...
    expect(wrapper.find('.app-course-compact-row__pct').text()).toBe('50%');
  });
  ...
  it('state=auto + zero → bar fill=0%', () => {
    ...
    expect(wrapper.find('.app-course-compact-row__pct').text()).toBe('0%');
  });
  ...
  it('has tabindex=0, role=button, aria-label=title on root', () => {
    const wrapper = mount(CourseCompactRow, { props: { course: base } });
    const root = wrapper.find('.app-course-compact-row');
  ...
  it('emits click with course on mouse click', async () => {
    const wrapper = mount(CourseCompactRow, { props: { course: base } });
    await wrapper.find('.app-course-compact-row').trigger('click');
  ...
  it('emits click on Enter keydown', async () => {
    const wrapper = mount(CourseCompactRow, { props: { course: base } });
    await wrapper.find('.app-course-compact-row').trigger('keydown', { key: 'Enter' });
    expect(wrapper.emitted('click')).toHaveLength(1);
  });

  it('emits click on Space keydown', async () => {
    const wrapper = mount(CourseCompactRow, { props: { course: base } });
    await wrapper.find('.app-course-compact-row').trigger('keydown', { key: ' ' });
    expect(wrapper.emitted('click')).toHaveLength(1);
  });

  it('does not emit click on other keys', async () => {
    const wrapper = mount(CourseCompactRow, { props: { course: base } });
    await wrapper.find('.app-course-compact-row').trigger('keydown', { key: 'Tab' });
  ...
  it('renders skeleton variant when loading=true', () => {
    ...
    expect(wrapper.find('.app-course-compact-row--loading').exists()).toBe(true);
    expect(wrapper.find('.app-course-compact-row__title').exists()).toBe(false);
  });

  it('loading skeleton is not focusable', () => {
    const wrapper = mount(CourseCompactRow, { props: { course: base, loading: true } });
    const root = wrapper.find('.app-course-compact-row--loading');
